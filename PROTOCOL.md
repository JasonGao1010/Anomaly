...
本轮完成数据、预测、评价和几何诊断基础的单帧化。主干网络、检测头、训练样本生成方式和训练损失尚未确定；后续应先完成残余失误诊断图谱，再根据真实证据决定方法设计。当前没有正式模型成绩，也没有对边界、稀疏或低矮条件作出保留、合并或放弃的实验裁决。

机器可读定义在 [protocol.json](protocol.json)，读取和必要语义检查由 [src/protocol.py](src/protocol.py) 完成。当前格式为 `stu-single-frame`，格式版本为 1。
...
`protocol.json` 保留已记录的原始压缩包摘要，供需要核对数据来源时使用。
...
PYTHONDONTWRITEBYTECODE=1 OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 \
  ...
  --predictions runs/predictions/<model> --output runs/evaluation/<model>
```
...
[src/profile.py](src/profile.py) 保留原有单帧统计算法，包括异常点数、距离、强度、最近正常点距离、正常邻域、同实例最近邻、可见实例尺寸、协方差形态和地面关系。几何量在当前扫描中计算，源点身份贯穿统计过程。
...
[src/profile_report.py](src/profile_report.py) 根据实际生成的逐帧记录、逐实例记录和分布累计结果导出 [profiles/real](profiles/real)。分位数来自完整加权经验分布，不平均各序列分位数。该目录保存：
...
进程数必须依据当时 CPU、内存和其他任务实测状态确定。少量帧检查可使用 `--sequence` 和 `--limit`，并指定独立的输出及表格目录，避免将子集结果覆盖到完整画像位置。
...
现有 `artifacts/calibration` 和 `artifacts/support` 保留为可选工具材料。它们的存储格式和来源校验用于读取既有几何资产，不规定新研究的训练样本分布。新训练生成方案应在残余失误诊断之后另行确定。
