...
\title{CReV: Verifying LiDAR Semantic Predictions\\with Held-Out Returns}
...
The evaluation follows the role of measurement evidence in CReV. Normal semantic accuracy measures its effect on known-class decisions. Real-anomaly detection tests whether it identifies unknown points that an appearance-based classifier assigns to known classes. High-recall false positives and inference cost then characterize the operating conditions under which this additional evidence is useful.
...
\label{sec:benchmark}
Table~\ref{tab:main} evaluates semantic accuracy and unknown detection together because measurement evidence influences both outputs. CReV and an independently trained appearance model use the same backbone, normal data, and training budget. Their comparison measures the change in perception obtained by introducing class-conditioned return verification.
...
CReV & \blank & \blank & \blank & \blank & \blank & \blank & \blank\\
\midrule
...
\label{sec:mechanism}
To isolate the role of measurement evidence in semantic learning, the separate-objectives control trains classification using appearance support while retaining the predictive objective, shared class parameters, and joint inference score. Its comparison with CReV tests the effect of using measurement support during class competition. The independently trained appearance model provides a common reference for examining which pointwise decisions change.
...
\section{Conclusion}
CReV connects LiDAR semantic decisions to predictions of measured range. Each known class receives appearance support from the full scan and measurement support from a context-conditioned distribution over a held-out return. Normal semantic labels train their joint support through class competition, while a likelihood objective fits the predictive distributions to normal measurements. The resulting class energy supplies both semantic labels and unknown scores, linking the recognition of known objects to the evidence supporting their classification.
\resultslot
