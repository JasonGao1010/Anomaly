...
\caption{A geometric departure can have a normal explanation. Left: a real scan contains a car above the road; colors indicate annotated classes and red marks one measured return. Right: extending the surrounding road along the same ray gives a farther intersection, while the car provides a surface at the observed range. The dashed road is a local plane fitted for this geometric illustration. SERVE learns to check every known class against both appearance and measured range, assessing unknown evidence through the strongest joint explanation.}
