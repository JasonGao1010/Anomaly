...
def get_extensions():
    ...
    # AJAE only uses farthest-point sampling. Building this official operator alone
    # avoids unrelated legacy renderers which do not link against CUDA 13.
    sources = [
        os.path.join(extensions_dir, "ext.cpp"),
        os.path.join(
            extensions_dir,
            "sample_farthest_points",
            "sample_farthest_points_cpu.cpp",
        ),
    ]
    source_cuda = [
        os.path.join(
...
setup(
    ...
    packages=find_packages(exclude=("configs", "tests", "tests.*", "docs.*")),
    install_requires=[],
