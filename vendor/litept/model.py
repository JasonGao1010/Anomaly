...
from .pointrope.pointrope_torch import PointROPE
...
def sparse_forward(module, tensor):
    # spconv 2.3.8 bypasses its autocast decorator in eval; cast all three operands together.
    if not module.training and torch.is_autocast_enabled("cuda"):
        dtype = torch.get_autocast_dtype("cuda")
        tensor = tensor.replace_feature(tensor.features.to(dtype))
        return module._conv_forward(False, tensor, module.weight.to(dtype),
                                    None if module.bias is None else module.bias.to(dtype),
                                    name=module.name, sparse_unique_name=module._sparse_unique_name,
                                    act_type=module.act_type, act_alpha=module.act_alpha, act_beta=module.act_beta)
        ...
        for k, module in self._modules.items():
            ...
            # Spconv module
                ...
                if isinstance(input, Point):
                    input.sparse_conv_feat = sparse_forward(module, input.sparse_conv_feat)
                    input.feat = input.sparse_conv_feat.features
                else:
                    input = sparse_forward(module, input)
            # PyTorch module
            else:
                if isinstance(input, Point):
...
class LitePT(PointModule):
    def __init__(
        ...
        # norm layers
        # V1 uses current-scan statistics in both modes; affine parameters remain learned.
        bn_layer = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01,
                           track_running_stats=norm_running_stats)
