...
class PointROPEAttention(PointModule):
    ...
    def forward(self, point):
        ...
        k = self.rope(k.transpose(0, 1)[None], pos)[0].transpose(0, 1)
        if "attention_windows" not in point:
            bounds = cu_seqlens.cpu().tolist()
            groups = {}
            for start, end in zip(bounds[:-1], bounds[1:]):
                groups.setdefault(end - start, []).append(start)
            point["attention_windows"] = [
                torch.tensor(starts, device=q.device)[:, None]
...
class Block(PointModule):
    ...
    def forward(self, point: Point):
        if self.recompute and self.training and torch.is_grad_enabled():
            # Blocks contain LayerNorm, never running-statistic BN. Recompute only
            # their activations; pooling/unpooling BN still runs exactly once.
            layout = dict(point)
