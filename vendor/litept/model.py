"""
LitePT model
standalone implementation, upstream commit 436d04801c8151faebe66a1b2d368a9711e7e6aa.
Local integration: Python PointROPE, explicit serialization order propagation,
autocast attention dtype and FP32 sparse kernels (spconv lacks BF16 support).
"""
import sys
        ...
        for k, module in self._modules.items():
            ...
            # Spconv module
                ...
                if isinstance(input, Point):
                    # spconv is outside autocast's dispatcher and requires matching dtypes.
                    with torch.autocast(input.feat.device.type, enabled=False):
                        sparse = input.sparse_conv_feat.replace_feature(input.feat.float())
                        input.sparse_conv_feat = module(sparse)
