 ...
 public:
  ...
  gdf_error prefetch(const int dev_id, cudaStream_t stream = 0)
  {
    ...
    if (cudaSuccess == status && isPtrManaged(hashtbl_values_ptr_attributes)) {
      ...
      cudaMemLocation const location{cudaMemLocationTypeDevice, dev_id};
...
#if CUDART_VERSION >= 13000
    cudaMemLocation const location{cudaMemLocationTypeDevice, dev_id};
    CUDA_TRY(cudaMemPrefetchAsync(this, sizeof(*this), location, 0, stream));
#else
   ...
   *
   * This function is invoked as the deleter of the `std::shared_ptr` returned
