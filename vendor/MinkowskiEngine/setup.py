...
def _append_existing_include(*candidates):
...
if CUDA_HOME is not False:  # False when not set, str otherwise
...
if BLAS is not False:  # False only when not set, str otherwise
    ...
    if BLAS_INCLUDE_DIRS is not False:
        include_dirs += BLAS_INCLUDE_DIRS
    if BLAS_LIBRARY_DIRS is not False:
        extra_link_args += [f"-Wl,-rpath,{BLAS_LIBRARY_DIRS}"]
else:
    # NumPy 2 removed numpy.distutils. Probe the linker-visible BLAS libraries.
    for blas in BLAS_LIST:
        library = "mkl_rt" if blas == "mkl" else blas
        ...
        raise ImportError(
            "No linker-visible BLAS library found. Pass --blas and optional "
            "--blas_include_dirs/--blas_library_dirs explicitly."
        )

# NumPy 2 no longer reports BLAS include paths. Prefer the active environment,
# then the standard Debian multiarch location used by the system BLAS fallback.
_append_existing_include(
    Path(sys.prefix) / "include",
    Path("/usr/include") / str(sysconfig.get_config_var("MULTIARCH")),
)
