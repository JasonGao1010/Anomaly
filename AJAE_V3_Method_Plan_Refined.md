...
正式val19标签按raw=2为异常、raw=0忽略、其余有效点为正常处理；训练中隔离原生raw=2的规则不适用于真实评价。官方主表保持2.5—50米（含端点）及“合格帧至少5个异常点”规则，按官方有效点全局合并排序，首先报告AP、AUROC和FPR95。困难组召回、正常误报、R@0.1%FPR、R@1%FPR及共享阈值下的精度/IoU单独作为补充评价，用于检验针对性贡献，不能替代官方三指标。
