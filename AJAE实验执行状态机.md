...
flowchart TB
  ...
  subgraph P1["Phase 1｜规范 OS1-128 射线身份"]
    E08["E08 槽位总数与空槽规律"]
    E09["E09 128 beam / elevation 恢复"]
    E10["E10 azimuth column 连续性"]
...
统计 slot/点的 elevation 分布并聚类/匹配到 128 个 beam，检查跨帧一致性。

**PASS 条件**

稳定得到 128 个 beam，跨帧映射一致。

**FAIL 条件**

beam 数量、顺序或 elevation 映射不稳定。

**状态转移**

- PASS → **E10**
- FAIL → **重建 beam assignment 后重跑 E09。**


## E10｜azimuth column 连续性
...
恢复 azimuth column，检查周期、单调/环绕连续性与跨帧稳定性。
...
- PASS → **E11**
- FAIL → **重建 azimuth indexing 后重跑 E10。**
...
判断 raw slot 能否安全绑定规范射线 r=(b,a)。

**建模 / 实施**

对同一候选 slot/ray 跨帧比较单位方向与 beam/column 身份；阈值须在正式运行前冻结。
