...
## E25-v3｜有限、精确、尺寸感知的目标—支撑关联

E25-v2正式FAIL永久保留。后续设计层归因冻结为`qualification specification defect`，不解释为normal-control构念失败。E25-v3已经冻结的关联结构为：类别允许的支撑语义不变；优先检查目标真实实例所在同一帧；同帧没有满足最终物理距离边界的支撑时，严格按$f-1,f+1,f-2,f+2$顺序扩展；所有距离始终相对于目标帧真实实例的世界坐标XY凸包计算；距离使用支撑锚点到闭合凸多边形的真实二维欧氏距离，凸包内部距离为0；不得使用逐半空间0.5米近似，不得执行无上限最近邻回退。
...
其中$D_{xy}$的唯一正式定义和$\alpha$均等待课题负责人根据只读物理可观察性诊断一次性确定。它们不得根据256/256生成是否通过、E45A匹配数或E46分类结果反向选择。在这两个量冻结以前，不重建E25-v3目标库，不运行control生成器，也不接触train/201。
...
当前停止点为：只读诊断完成，$D_{xy}$和$\alpha$未冻结，E25-v3目标库与生成器均未运行；E26-v2、E46和E48继续锁定。
