...
一个异常代理由 $1$ 至 $5$ 个 primitive 组合：

$$
G=P_1\oplus P_2\oplus\cdots\oplus P_n
$$
...
若组合结果分裂成多个互不相连的组件，则拒绝该样本，而不是把多个物体伪装成一个实体。
...
训练阶段至少混合超二次曲面、CSG 和连续形变；开发诊断中保留至少一种训练时完全没有使用的程序化形状机制。
