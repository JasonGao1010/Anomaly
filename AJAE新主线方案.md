...
E43已冻结并联合读取E37跨窗口一致性证据和E39五帧共享trace，复核重复渲染与window身份后，逐实体计算相邻帧$N_{vis}$变化率、出现/消失计数和$V=0,\ldots,5$分层。实现已通过46项完整回归，正式结果尚未产生。
...
$$
\boxed{E42\ \text{PASS};\quad E43\ \text{FROZEN AND RUNNING};\quad E44\ \text{LOCKED}}
$$
...
E27–E42已关闭；E36-v1 FAIL按设计冲突保留，当前正在执行E43，E44尚未启动。
